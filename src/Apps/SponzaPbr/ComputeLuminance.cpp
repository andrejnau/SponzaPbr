...
ComputeLuminance::ComputeLuminance(RenderDevice& device, const Input& input, int width, int height)
    ...
    , m_width(width)
    , m_height(height)
    , m_HDRLum1DPass_CS(device)
    , m_HDRLum2DPass_CS(device)
    , m_HDRApply(device)
{
    CreateBuffers();
}
...
void ComputeLuminance::GetLum2DPass_CS(RenderCommandList& command_list, size_t buf_id, uint32_t thread_group_x, uint32_t thread_group_y)
{
    ...
    command_list.Attach(m_HDRLum2DPass_CS.cs.cbv.cb, m_HDRLum2DPass_CS.cs.cbuffer.cb);

    command_list.Attach(m_HDRLum2DPass_CS.cs.uav.result, m_use_res[buf_id]);
    command_list.Attach(m_HDRLum2DPass_CS.cs.srv.data, m_input.hdr_res);
...
void ComputeLuminance::GetLum1DPass_CS(RenderCommandList& command_list, size_t buf_id, uint32_t input_buffer_size, uint32_t thread_group_x)
{
    ...
    command_list.Attach(m_HDRLum1DPass_CS.cs.cbv.cb, m_HDRLum1DPass_CS.cs.cbuffer.cb);

    command_list.Attach(m_HDRLum1DPass_CS.cs.srv.data, m_use_res[buf_id - 1]);
    command_list.Attach(m_HDRLum1DPass_CS.cs.uav.result, m_use_res[buf_id]);

    command_list.Dispatch(thread_group_x, 1, 1);

    command_list.Attach(m_HDRLum1DPass_CS.cs.uav.result);
}
